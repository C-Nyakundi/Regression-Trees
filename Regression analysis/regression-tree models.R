...
# The code generates predictions (pred) for the test dataset using the regression tree model (tree). The predict() function is applied to the model object (tree) and specifies the new data (test) for which predictions are needed.
# After execution, the pred object contains the predicted values based on the regression tree model for the observations in the test dataset.

# Computing the RSME 
Metrics::rmse(actual = test$Weight..kg., predicted = pred)
# The Metrics::rmse() function calculates the root mean squared error (RMSE) between the actual weights (test$Weight..kg.) from the test dataset and the predicted weights (pred) generated by the regression tree model.

# RMSE is a measure of the differences between predicted and actual values, where lower values indicate better predictive performance. This function computes the square root of the average of the squared differences between actual and predicted values.
